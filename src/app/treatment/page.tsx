...
export default function TreatmentPage() { return <><PageHero eyebrow="施術のご案内" title="施術・メニュー" lead="痛みの場所だけでなく、身体全体の状態や日常の動き方まで確認して施術をご提案します。" /><Breadcrumb current="施術・メニュー" path="/treatment" /><section className="section"><div className="container"><SectionHeading eyebrow="施術メニュー" title="状態に合わせた施術" /><div className="menu-list">{treatments.map((item, i) => <article key={item.title}><Image src={item.image} alt={`${item.title}の施術イメージ`} width={700} height={520} /><div><span>0{i + 1}</span><h2><Ja>{item.title}</Ja></h2><p><Ja>{item.text}</Ja></p><dl className="menu-details"><div><dt>主なお悩み</dt><dd><Ja>{menuDetails[i][0]}</Ja></dd></div><div><dt>施術前の確認</dt><dd><Ja>{menuDetails[i][1]}</Ja></dd></div><div><dt>施術の考え方</dt><dd><Ja>{menuDetails[i][2]}</Ja></dd></div></dl></div></article>)}</div><p className="note menu-price-note"><Ja>メニューや料金は、ご予約ページの最新情報をご確認ください。</Ja></p></div></section><section className="section pale"><div className="container"><SectionHeading eyebrow="保険施術" title="保険施術について" lead="健康保険の取扱いは、症状や受傷原因などによって異なります。詳しくは来院時または予約時にご確認ください。" /><p className="note"><Ja>施術内容・料金は、お身体の状態や保険適用の有無によって異なります。来院時に事前にご説明します。</Ja></p></div></section><ReservationCTA /></>; }
